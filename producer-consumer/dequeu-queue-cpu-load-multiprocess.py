...
def consume(shared_q, consumer_id):
    while True:
        packet = shared_q.get()
...
def main():
    ...
    producer = multiprocessing.Process(target=produce, args=(shared_q, NUM_CONSUMERS,))
    producer.start()
    ...
    for i in range(NUM_CONSUMERS):
        consumer = multiprocessing.Process(target=consume, args=(shared_q, i,))
        consumer.start()
